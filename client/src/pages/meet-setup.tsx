...
export default function MeetSetup() {
  ...
  return (
    <div className="px-6 py-8 space-y-6 max-w-4xl">
      ...
      <Card data-testid="card-sponsor-directory">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
          ...
          <CardDescription>
            Set the path to a folder containing sponsor images for the Sponsor Reel display
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="C:\Users\YourName\Desktop\Sponsors"
            />
            <p className="text-xs text-muted-foreground">
              Full path to a folder containing sponsor images (PNG, JPG, GIF, WebP, SVG)
            </p>
          </div>
          <div className="flex gap-2">
            <Button
              onClick={() => saveSponsorDirMutation.mutate({ sponsorDir: sponsorDir || null })}
              disabled={!hasSponsorDirChanges || saveSponsorDirMutation.isPending}
              size="sm"
            >
              Save Directory
            </Button>
            <Button
              ...
              disabled={!sponsorDir && !meet?.sponsorDir}
            >
              Clear
            </Button>
          </div>
      ...
      <Card data-testid="card-lynx-files">
        <CardHeader>
          ...
          <CardDescription>
            Directory containing LIF and LFF files from FinishLynx and FieldLynx.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
          ...
          {ingestionSettings?.lynxFilesEnabled && ingestionSettings?.lynxFilesDirectory && (
            <div className="flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
              <Check className="w-4 h-4" />
              <span>Directory configured</span>
            </div>
          )}
...
function RecordBooksSection() {
  ...
  const { data: recordBooks = [], isLoading } = useQuery<RecordBookWithRecords[]>({
    ...
    queryFn: async () => {
      const r = await fetch('/api/record-books?all=true');
      if (!r.ok) return [];
