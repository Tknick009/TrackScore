...
export function FieldTransitionRenderer({
  ...
  // PORT ISOLATION: Extract ONLY the data for THIS device's assigned port.
  // This intermediate useMemo returns a STABLE reference when other ports' data changes,
  // which prevents the downstream useEffect from re-running and false-triggering the curtain.
  // Without this, every port update caused a new mergedLiveData array, firing the curtain
  // detection logic even when this port's data hadn't changed.
  const relevantPortData = useMemo(() => {
    if (deviceFieldPort && liveEventDataByPort) {
      return liveEventDataByPort[deviceFieldPort] || null;
    }
    return null;
  }, [deviceFieldPort, liveEventDataByPort]);

  // Build the data sources array from the stable port-specific reference.
  ...
  const mergedLiveData = useMemo(() => {
    ...
    if (relevantPortData) {
      const pd = relevantPortData as any;
      if (!pd.isStandings && pd.mode !== 'field_standings') {
        allSources.push(relevantPortData);
      }
    } else if (!deviceFieldPort && liveData) {
      // Fallback ONLY when no port is assigned at all (not just "no data yet")
      const ld = liveData as any;
      if (!ld.isStandings && ld.mode !== 'field_standings') allSources.push(liveData);
    }

    return allSources;
  }, [liveData, relevantPortData, deviceFieldPort]);
