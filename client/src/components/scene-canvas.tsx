...
// FieldTransitionRenderer — curtain-wipe overlay for a single FieldLynx port.
// Place this as a transparent scene object over the area you want the wipe to cover.
// Config: fieldPort (number), curtainColor (CSS color, falls back to backgroundColor style)
type CurtainPhase = 'idle' | 'curtain' | 'split' | 'reveal';

function FieldTransitionRenderer({
  fieldPort,
  curtainColor,
}: {
  fieldPort: number;
  ...
  // Curtain slides in from the right edge; on split it splits L/R; on reveal both halves exit
  ...
  const leftPanelStyle: React.CSSProperties = {
    ...panelBase,
    left: 0,
    transform: phase === 'reveal' || phase === 'idle'
      ? 'translateX(-100%)'
      : 'translateX(0)',
  };
  ...
  // Full-width entry curtain slides in from right, used during 'curtain' phase only
  ...
  const renderContent = () => {
    switch (object.objectType) {
      ...
      case "field-transition": {
        const ftPort = componentConfig.fieldPort as number | undefined;
