  ...
  try {
    ...
    for (const row of athleteData) {
      try {
        ...
        const firstName = row.First || row.FirstName || row.first || "";
        const lastName = row.Last || row.LastName || row.last || "";
        ...
        console.error(`Error importing athlete:`, err);
      }
    }
  ...
  try {
    ...
    for (const row of eventData) {
      try {
        ...
        // Determine gender from event name or division
        let gender = "mixed";
        if (eventName.toLowerCase().includes("men") || eventName.toLowerCase().includes("boy")) gender = "men";
