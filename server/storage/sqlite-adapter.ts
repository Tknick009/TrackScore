...
export class SQLiteStorage implements IStorage {
  ...
  private mapEventRow(row: any): Event {
    return {
      ...
      prelimDate: row.prelim_date ? new Date(row.prelim_date) : null,
      prelimTime: row.prelim_time,
      prelimSessionName: row.prelim_session_name,
      finalDate: row.final_date ? new Date(row.final_date) : null,
      finalTime: row.final_time,
      finalSessionName: row.final_session_name,
  ...
  async createMeet(meet: InsertMeet): Promise<Meet> {
    ...
    this.db.prepare(`
      INSERT INTO meets (id, season_id, name, location, start_date, end_date, status, track_length, logo_url, meet_code, mdb_path, auto_refresh, refresh_interval, primary_color, secondary_color, accent_color, text_color, sponsor_dir)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      meet.seasonId ?? null,
