  ...
  lynxListener.on('field-mode-change', async (eventNumber, mode, data) => {
    ...
    try {
      ...
      if (fieldActiveMeetId && fieldMeetFiltered.length === 0 && allFieldMatchEvents.length > 0) {
        console.warn(`[Lynx] Field event ${eventNumber} found in other meets but NOT in active meet ${fieldActiveMeetId} — falling back to all matches`);
      }
  ...
  lynxListener.on('start-list', async (eventNumber, heat, entries, metadata) => {
    ...
    try {
      ...
      if (activeMeetId && meetFilteredEvents.length === 0 && allMatchEvents.length > 0) {
        console.warn(`[Lynx] Start list event ${eventNumber} found in other meets but NOT in active meet ${activeMeetId} — falling back to all matches`);
      }
