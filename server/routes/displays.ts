    ...
    try {
      ...
      const enrichedEntries = displayEntries.map((entry, index) => {
        ...
        return {
          ...
          statusCode: isKnownStatus ? dqCode : (entry.isDisqualified ? 'DQ' : null),
      ...
      if (standings.length > 0) {
        teamEntries = standings
          ...
          .map((s: any) => ({
            ...
            time: String(Math.round(((s.totalPoints || 0) + Number.EPSILON) * 100) / 100),
            mark: String(Math.round(((s.totalPoints || 0) + Number.EPSILON) * 100) / 100),
            points: Math.round(((s.totalPoints || 0) + Number.EPSILON) * 100) / 100,
